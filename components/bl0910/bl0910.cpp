...
static const float BL0910_PREF = (40.4125f * 33.0f * 220.0f * 1000.0f / 2000.0f) / (1.097f * 1.097f * 132.0f);  // magic3 * Rburden1 * Rburden2 / Vref * Vref * PT primary resistance
...
uint8_t BL0910::checksum_calc(uint8_t* data)
{
  uint8_t checksum = 0;
  for (int i = 0; i < 5; i++)
  {
...
bool BL0910::checksum_check(uint8_t* data)
{
  uint8_t checksum = 0;
  for (int i = 0; i < 5; i++)
  {
  ...
  checksum = checksum & 0xFF;
  checksum = checksum ^ 0xFF; // bit invert
...
void BL0910::bl0910_write_data(uint8_t addr, uint8_t data_h, uint8_t data_m, uint8_t data_l) {
  uint8_t packet[6] = { 0 };
...
int32_t BL0910::bl0910_read_data(uint8_t addr) {
  int32_t data = 0;
  uint8_t packet[6] = { 0 };
  ...
  packet[1] = addr;
  this->enable();
  this->write_array(packet, 2);
  this->read_array(&packet[2], 4);
  this->disable();

  if (checksum_check(packet)) {
...
void BL0910::update() {
  ...
  float current1 = (float)bl0910_read_data(BL0910_REG_RMS_1) / BL0910_IREF;
  float current2 = (float)bl0910_read_data(BL0910_REG_RMS_2) / BL0910_IREF;
  float current3 = (float)bl0910_read_data(BL0910_REG_RMS_3) / BL0910_IREF;
  float current4 = (float)bl0910_read_data(BL0910_REG_RMS_4) / BL0910_IREF;
  float current5 = (float)bl0910_read_data(BL0910_REG_RMS_5) / BL0910_IREF;
  float current6 = (float)bl0910_read_data(BL0910_REG_RMS_6) / BL0910_IREF;
  float current7 = (float)bl0910_read_data(BL0910_REG_RMS_7) / BL0910_IREF;
  float current8 = (float)bl0910_read_data(BL0910_REG_RMS_8) / BL0910_IREF;
  float current9 = (float)bl0910_read_data(BL0910_REG_RMS_9) / BL0910_IREF;
  float current10 = (float)bl0910_read_data(BL0910_REG_RMS_10) / BL0910_IREF;
   
  float watt1 = (float)bl0910_read_data(BL0910_REG_WATT_1) / BL0910_PREF;
  float watt2 = (float)bl0910_read_data(BL0910_REG_WATT_2) / BL0910_PREF;
  float watt3 = (float)bl0910_read_data(BL0910_REG_WATT_3) / BL0910_PREF;
  float watt4 = (float)bl0910_read_data(BL0910_REG_WATT_4) / BL0910_PREF;
  float watt5 = (float)bl0910_read_data(BL0910_REG_WATT_5) / BL0910_PREF;
  float watt6 = (float)bl0910_read_data(BL0910_REG_WATT_6) / BL0910_PREF;
  float watt7 = (float)bl0910_read_data(BL0910_REG_WATT_7) / BL0910_PREF;
  float watt8 = (float)bl0910_read_data(BL0910_REG_WATT_8) / BL0910_PREF;
  float watt9 = (float)bl0910_read_data(BL0910_REG_WATT_9) / BL0910_PREF;
  float watt10 = (float)bl0910_read_data(BL0910_REG_WATT_10) / BL0910_PREF;
   
  float energy1 = (float)bl0910_read_data(BL0910_REG_CF_CNT_1) / BL0910_EREF;
  float energy2 = (float)bl0910_read_data(BL0910_REG_CF_CNT_2) / BL0910_EREF;
  float energy3 = (float)bl0910_read_data(BL0910_REG_CF_CNT_3) / BL0910_EREF;
  float energy4 = (float)bl0910_read_data(BL0910_REG_CF_CNT_4) / BL0910_EREF;
  float energy5 = (float)bl0910_read_data(BL0910_REG_CF_CNT_5) / BL0910_EREF;
  float energy6 = (float)bl0910_read_data(BL0910_REG_CF_CNT_6) / BL0910_EREF;
  float energy7 = (float)bl0910_read_data(BL0910_REG_CF_CNT_7) / BL0910_EREF;
  float energy8 = (float)bl0910_read_data(BL0910_REG_CF_CNT_8) / BL0910_EREF;
  float energy9 = (float)bl0910_read_data(BL0910_REG_CF_CNT_9) / BL0910_EREF;
  float energy10 = (float)bl0910_read_data(BL0910_REG_CF_CNT_10) / BL0910_EREF;
  ...
  if (voltage_sensor_ != nullptr) {
   ...
   if (current_sensor_1_ != nullptr) {
     current_sensor_1_->publish_state(current1);
   }
   if (current_sensor_2_ != nullptr) {
     current_sensor_2_->publish_state(current2);
   }
   if (current_sensor_3_ != nullptr) {
     current_sensor_3_->publish_state(current3);
   }
   if (current_sensor_4_ != nullptr) {
     current_sensor_4_->publish_state(current4);
   }
   if (current_sensor_5_ != nullptr) {
     current_sensor_5_->publish_state(current5);
   }
   if (current_sensor_6_ != nullptr) {
     current_sensor_6_->publish_state(current6);
   }
   if (current_sensor_7_ != nullptr) {
     current_sensor_7_->publish_state(current7);
   }
   if (current_sensor_8_ != nullptr) {
     current_sensor_8_->publish_state(current8);
   }
   if (current_sensor_9_ != nullptr) {
     current_sensor_9_->publish_state(current9);
   }
   if (current_sensor_10_ != nullptr) {
     current_sensor_10_->publish_state(current10);
   }
   
   if (power_sensor_1_ != nullptr) {
     power_sensor_1_->publish_state(watt1);
   }
   if (power_sensor_2_ != nullptr) {
     power_sensor_2_->publish_state(watt2);
   }
   if (power_sensor_3_ != nullptr) {
     power_sensor_3_->publish_state(watt3);
   }
   if (power_sensor_4_ != nullptr) {
     power_sensor_4_->publish_state(watt4);
   }
   if (power_sensor_5_ != nullptr) {
     power_sensor_5_->publish_state(watt5);
   }
   if (power_sensor_6_ != nullptr) {
     power_sensor_6_->publish_state(watt6);
   }
   if (power_sensor_7_ != nullptr) {
     power_sensor_7_->publish_state(watt7);
   }
   if (power_sensor_8_ != nullptr) {
     power_sensor_8_->publish_state(watt8);
   }
   if (power_sensor_9_ != nullptr) {
     power_sensor_9_->publish_state(watt9);
   }
   if (power_sensor_10_ != nullptr) {
     power_sensor_10_->publish_state(watt10);
   }
   
   
   if (energy_sensor_1_ != nullptr) {
     energy_sensor_1_->publish_state(energy1);
   }
   if (energy_sensor_2_ != nullptr) {
     energy_sensor_2_->publish_state(energy2);
   }
 ...
 void BL0910::dump_config() {  // NOLINT(readability-function-cognitive-complexity)
   ...
   LOG_SENSOR("", "Current 5", this->current_sensor_5_);
   LOG_SENSOR("", "Current 6", this->current_sensor_6_);
   LOG_SENSOR("", "Current 7", this->current_sensor_7_);
   LOG_SENSOR("", "Current 8", this->current_sensor_8_);
   LOG_SENSOR("", "Current 9", this->current_sensor_9_);
   LOG_SENSOR("", "Current 10", this->current_sensor_10_);
   ...
   LOG_SENSOR("", "Power 5", this->power_sensor_5_);
   LOG_SENSOR("", "Power 6", this->power_sensor_6_);
   LOG_SENSOR("", "Power 7", this->power_sensor_7_);
   LOG_SENSOR("", "Power 8", this->power_sensor_8_);
   LOG_SENSOR("", "Power 9", this->power_sensor_9_);
   LOG_SENSOR("", "Power 10", this->power_sensor_10_);
   ...
   LOG_SENSOR("", "Energy 5", this->energy_sensor_5_);
   LOG_SENSOR("", "Energy 6", this->energy_sensor_6_);
   LOG_SENSOR("", "Energy 7", this->energy_sensor_7_);
   LOG_SENSOR("", "Energy 8", this->energy_sensor_8_);
   LOG_SENSOR("", "Energy 9", this->energy_sensor_9_);
   LOG_SENSOR("", "Energy 10", this->energy_sensor_10_);
